...
CT.Temp.RNF <- melt(CT.Temp[CT.Temp$Site.Code=="RNF",])
Tmin <- as.data.frame(cast(CT.Temp.RNF, Sampling.Unit.Name ~ Sampling.Period ~ variable)[,,1])
names(Tmin) <- paste0("Tmin.",2010:2013)
Tmax <- as.data.frame(cast(CT.Temp.RNF, Sampling.Unit.Name ~ Sampling.Period ~ variable)[,,2])
names(Tmax) <- paste0("Tmax.",2010:2013)
Tvar <- round(as.data.frame(cast(CT.Temp.RNF, Sampling.Unit.Name ~ Sampling.Period ~ variable)[,,3]),2)
names(Tvar) <- paste0("Tvar.",2010:2013)
RNF.CovariateData <- cbind(Tmin, Tmax, Tvar)
